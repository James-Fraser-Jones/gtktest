...
main = do
  ...
  mkBtn "+" (operator (\str -> Addition str)) >>= attach 3 6 1 1
  mkBtn "–" (operator (\str -> Subtraction str)) >>= attach 3 5 1 1
  mkBtn "*" (operator (\str -> Multiplication str)) >>= attach 3 4 1 1
  mkBtn "÷" (operator (\str -> Division str)) >>= attach 3 3 1 1
...
mkButton :: IORef Value -- ^ 'IORef' to calculator state
